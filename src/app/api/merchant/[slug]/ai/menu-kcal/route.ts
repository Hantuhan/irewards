...
const bodySchema = z.object({
  ...
  /** When true, ask AI even if a local preset match exists */
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await askMenuKcal({
      name: body.name,
    ...
    const message = err instanceof Error ? err.message : "Failed to estimate kcal";
